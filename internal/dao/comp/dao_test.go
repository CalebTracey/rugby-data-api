...
func TestDAO_GetAllLeaderboardData(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	mockDao := dbmocks.NewMockDAOI(ctrl)
	mockMapper := dbmocks.NewMockMapperI(ctrl)
	...
	tests := []struct {
		...
		},
		{
			...
			ctx:      context.Background(),
			mockRows: mockRows,
			query:    fmt.Sprintf(psql.AllLeaderboardsQuery),
			wantResp: models.PSQLLeaderboardDataList(nil),
			mockErr:  errors.New("db error"),
			wantErr: &response.ErrorLog{
				StatusCode: "500",
				RootCause:  "db error",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockDao.EXPECT().FindAll(tt.ctx, tt.query).
				DoAndReturn(func(ctx context.Context, query string) (*sqlmock.Rows, *response.ErrorLog) {
					if tt.expectDbError {
						return mockRows, tt.wantErr
					}
					return mockRows, nil
				})
			if !tt.expectDbError {
				mockMapper.EXPECT().MapPSQLRowsToLeaderboardData(gomock.Any()).Return(tt.wantResp, tt.wantMapErr)
			}
			gotResp, gotErr := s.GetAllLeaderboardData(tt.ctx)
			if !reflect.DeepEqual(gotResp, tt.wantResp) {
				t.Errorf("GetAllLeaderboardData() gotResp = %v, want %v", gotResp, tt.wantResp)
			}
			if !reflect.DeepEqual(gotErr, tt.wantErr) {
				t.Errorf("GetAllLeaderboardData() gotErr = %v, want %v", gotErr, tt.wantErr)
			}
		})
	}
}
